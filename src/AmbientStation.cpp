...
public:
  ...
  Client(boost::asio::io_context &io_context, const std::string &server,
         const std::string &tg_server, const std::string tg_port) :
          ...
          to_socket_(io_context, boost::asio::ip::udp::v4()),
          to_resolver_{io_context}
  {
    try
    {
      if ( verbose )
      { 
        std::cerr << "Resolve: " << tg_server << ":" << tg_port;
      }
      endpoints_ = to_resolver_.resolve(udp::v4(), tg_server, tg_port);
    ...
    {
      std::cerr << "Client constructor Exception:" << e.what() << std::endl;
    }
  }
...
private:
  ...
  void SendData()
  {
    ...
    try
    {
      std::size_t sz = to_socket_.send_to(
  ...
  try
  {
    if (argc < 5 || argc > 6)
    {
      std::cerr << "Usage: " << argv[0]
                << " <station-ip> <influxdb-ip> <influxdb-port> <sample-time-sec> [verbose]\n";
      std::cerr << "Example:\n";
      std::cerr << "   " << argv[0] << " 192.168.3.72 localhost 8089 60 verbose"
                << std::endl;
      return 1;
    }
    if (argc == 6 && !strcmp(argv[5], "verbose"))
      verbose = true;
    station_ip = argv[1];
    telegraf_ip = argv[2];
